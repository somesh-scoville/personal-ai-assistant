...
def my_agent_function(prompt: str, user_name: str, thread_name: str) -> str:
...
def main(user_name: str):
    ...
    st.markdown(
        "<h1 style='text-align: center; font-size: 50px;'>Chat Assistant</h1>",
    ...
    with st.sidebar:
        ...
        selected_thread = st.selectbox(
            "Choose a thread", options=list(st.session_state.chat_threads.keys())
        )
    ...
    if prompt := st.chat_input("What's on your mind?"):
        ...
        st.session_state.chat_threads[st.session_state.current_thread].append(
